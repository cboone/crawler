...
- Installing: `go get github.com/cboone/strider`
...
- The update workflow: `STRIDER_UPDATE=1 go test ./...` (truthy values are exact lowercase matches: `1`, `true`, `yes`), reviewing changes with `git diff testdata/`
...
- CI considerations: never run with `STRIDER_UPDATE=1` in CI
...
- tmux version too old: minimum 3.0, `t.Skip` for auto-detected vs `t.Fatal` for explicitly configured (`WithTmuxPath` or `STRIDER_TMUX`)
- Configuring the tmux path: resolution order (WithTmuxPath > STRIDER_TMUX > PATH)
...
- Socket path length: Unix 104/108 char limit, strider truncates sanitized name to 60 chars
...
- Debugging tips: `go test -run TestName -v`, `STRIDER_TMUX` for specific tmux builds
...
- `strider.go` — Open, WaitFor, WaitExit, Resize, Scrollback, diagnostics formatting
...
- `strider_test.go` — real usage patterns to adapt as examples
